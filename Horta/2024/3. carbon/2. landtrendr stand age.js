...
var ceoPoints = ee.FeatureCollection('users/ingperezescobar/TerraBio/ReforesTerra/data/2023/ceo-TerraBio--Reforesterra-2022-sample-data-2024-05-17')
...
Export.table.toDrive({
  ...
  fileNamePrefix: dealName + '_' + reportYear + '_combinedStandAge'+studyEndYear+'_20240519', 
  description: dealName + '_' + reportYear + '_combinedStandAge2'+studyEndYear+'_20240519',  
  fileFormat: 'CSV'
});
...
Export.image.toDrive({
  image: standAgeImage, 
  description: description + '_20240519', //!!! input date that the image is exported
  dimensions: null, 
  region: aoi, 
  scale: 30, 
  crs: 'EPSG:4326', 
...
Export.image.toAsset({
  image: standAgeImage, 
  description: description + '_20240519',  //!!! input date that the image is exported
  assetId: baseOutFolder + description + '_20240519', 
...
function identifyStandAgeFromStableForest(yr){
  ...
  // stand age = age in standAgeValue + (current year - 1985) 
  // returns: image
  ...
  //stable forest stand age should be (current year-1985)+standAgeValue (≤83)
...
function identifyStandAgeFromStableForestIntermediate(img){
  ...
  // stand age = standAgeValue + (current year - 1985)
  // returns: image
  ...
  //stable forest stand age should be (current year-1985)+standAgeValue
  var stableStandAge = ee.Number(standAgeValue).add(currentYear.subtract(forestStartYear)); 
...
function identifyStandAgeFromLtGainAndCurrentForest(img,yr){ 
  ...
  var modifiedStandAge = ee.Image(0) //image of zeroes 
    .where(currentStandAge, currentStandAge) //carry over ages from currentStandAge (stable forest)
      ...
      .and(currentLossGainOrder.neq(2)), futureRegrowthAge) //and gain NOT followed by loss
        .rename('standAge');
...
function identifyStandAgeFromLtGainAndCurrentNonForest(img,yr){
  ...
  // get stand age from ltAnnualRegrowth
  var currentLtGainStandAge = ltAnnualRegrowth.filterDate(startDate,startDate.advance(1, 'second')).first();
  
  // get order of events for current year (loss after gain, gain after loss)
  var currentLossGainOrder = ltLossGainOrder.filterDate(startDate,startDate.advance(1, 'second')).first();
  
  var modifiedStandAge = ee.Image(0)
    .where(currentStandAge, currentStandAge)
      .where(currentForestMask.eq(0) //no forest 
        .and(currentStandAge.eq(0)) //no stand age yet assigned, not necessary here
        .and(currentLtGainStandAge) //and regrowth calculated (so, there was previous gain) 
        .and(currentLossGainOrder.neq(2)), currentLtGainStandAge) //and gain NOT followed by loss
          .rename('standAge');
...
function makeAnnualLtRegrowth(yearList,ltGain,ltLoss){
  ...
  var landtrendrRegrowth = yearList.map(function(yr){
    ...
    var gainLossDiff = ee.Image(0).where(ltYearGain.gt(ltYearLoss),ltYearGain.subtract(ltYearLoss)); //difference
