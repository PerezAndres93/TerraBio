...
//CEO
var ceoRaw = gp.layers.ceo;  //213 elements
print('ceoRaw',ceoRaw);
...
var forestInQuestion = propNames.filter(ee.Filter.stringContains('item', 'Forest / canopy-covered LULC in')).get(0);  //!*! updated
...
var stableFC = ceo
      ...
      .filter(ee.Filter.eq(forestInQuestion, 1))
...
var nonForestFC = ceo
  ...
  .filter(ee.Filter.eq(forestInQuestion, 0))
  .filter(ee.Filter.neq('Most RECENT canopy-cover LOSS '+studyStartYear+'-'+studyEndYear, 'stable forest '+studyStartYear+'-'+studyEndYear))
  .filter(ee.Filter.eq('Canopy-cover GAIN '+studyStartYear+'-2021', 'No Canopy Cover Gain '+studyStartYear+'-'+studyEndYear));
print('nonForestFC',nonForestFC);
  
var deforestedFC = ceo
  .filter(ee.Filter.neq('Forest establishment year', null))
  .filter(ee.Filter.eq(forestInQuestion, 1))
...
var gainedFC = ceo
  .filter(ee.Filter.eq(forestInQuestion, 0))
  .filter(ee.Filter.eq('Canopy-cover GAIN '+studyStartYear+'-2021', 'Canopy Cover Gain (LULC change) '+studyStartYear+'-'+studyEndYear))
...
print('fc',fc);
var fcFinal = fc.distinct('plotid');
