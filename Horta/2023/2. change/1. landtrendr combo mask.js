...
var counterfactual = gp.layers.counterfactuals
...
var samzParams = gp.params.samzNeutral;
...
//Map.addLayer(changeImgGain.select(['yod']).clip(interventions), yodVizParms, 'Year of Detection (gain)',false);


// Visualize FNF mask //

var fnfVis = {min:0, max:1, palette:['grey','green']};

for(var i = 2017; i < 2023; i++){
  var fnfLabel = 'FNF ' + i.toString();
  Map.addLayer(fnfCombo.select('fnf'+i).clip(aoi), fnfVis, fnfLabel, false);
}
...
//Map.addLayer(outlineInterventions, {palette: paletteCiat.colors.colorInterventions}, 'Interventions');
