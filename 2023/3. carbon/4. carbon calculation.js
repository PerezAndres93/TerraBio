...
var syntropic = gp.layers.interventions;
//var counterfactuals = gp.layers.counterfactuals; // commented by Joge
var reference = gp.layers.reference;
//var regeneration = gp.layers.regeneration; // commented by Joge
...
syntropic = syntropic.map(function(f){return f.set('carbon_id','Agroforestry Latin America and the Caribbean')});
//counterfactuals = counterfactuals.map(function(f){return f.set('carbon_id','Planted forest broadleaf excl euc and teak (tropical humid)')});
...
farmsUndesignated = farmsUndesignated.map(function(f){return f.set('carbon_id','Planted forest broadleaf excl euc and teak (tropical humid)')});
print("farmsUndesignated",farmsUndesignated)
//regeneration = regeneration.map(function(f){return f.set('carbon_id','Planted forest broadleaf excl euc and teak (tropical humid)')});

/////////////// compile into one feature collection //////////////////

var landUses = ee.FeatureCollection([syntropic,
                                     //counterfactuals,
                                     reference,
                                     farmsUndesignated]).flatten();
...
////////////// modify forest mask so that syntropic, regeneration, and reference areas are given '1' values

var protectedAll = reference
      //note: mapping over features so can retain properties as oppsed to using .geometry 
      .map(function (f) { 
        var matrix = f
          .union({'right': syntropic.geometry(), 'maxError': 1})
          //.union({'right': regeneration.geometry(), 'maxError': 1});
...
//=========================================//
// calculate annual carbon sum by land use //
//=========================================//
...
//print(prepFarms);
var prepSyntropic = prepLu(syntropic);
//print(prepInterventions);
//var prepCounterfactuals = prepLu(counterfactuals); // commented by Joge
...
//var prepRegeneration = prepLu(regeneration);
...
var outlineIntv = empty.paint({
  featureCollection: syntropic,
  color: 1,
  width: 2
});
Map.addLayer(outlineIntv, {palette: paletteCiat.colors.colorIntervention}, 'Syntropic',false); //yellow CIAT
...
var luCarbonSum = final.filter(ee.Filter.stringContains('land_use','farms_2023'));
